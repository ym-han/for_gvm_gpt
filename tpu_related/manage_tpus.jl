...
function tpunm_from_jl_idx(idx, tpu_prefix)
  # where idx is 1, 2, ... , 7
  py_idx = idx - 1
...
function query_cmd_from_jl_idx(idx, qdict_idx_range, config_fnm)
...
function run_query_cmd(idx, tpu_prefix, cmd_maker)
  tpu_nm = tpunm_from_jl_idx(idx, tpu_prefix)
  cmd_str = cmd_maker(idx)

  run_on_tpu(tpu_nm, cmd_str)
end 

function git_pull_on_tpus(tpu_nms)
  @time begin
...
function map_idx_cmd_over_tpus(cmd_maker, tpu_prefix, n_tpus)
  makeshift_jl_idxes = 1:n_tpus
  
  @time begin
     asyncmap(i -> run_query_cmd(i, tpu_prefix, cmd_maker), makeshift_jl_idxes)
  end
end
...
function example_of_querying_gpt()
  ...
  round2_query_cmd_maker(idx) = query_cmd_from_jl_idx(idx, round_2_qds_idxes, "config_rest_of_top100_step1700.json")
  ...
  map_idx_cmd_over_tpus(round2_query_cmd_maker, tpu_prefix, n_tpus)
end
